...
class gender_recognition:
    ...
    def get_params(self):
        ...
        self.sub_image_raw_topic_name          = rospy.get_param('sub_image_raw_topic_name',          '/image')
        ...
        self.path_to_save_image                = rospy.get_param('path_to_save_image',                '/home/zmx/catkin_ws/src/kamerider_image/kamerider_image_detection/test_images/image_capture.jpg')
        self.path_to_save_result               = rospy.get_param('path_to_save_result',               '/home/zmx/catkin_ws/src/kamerider_image/kamerider_image_detection/result/gender_recognition_result.jpg')
        #self.path_to_save_image                = rospy.get_param('path_to_save_image',                '../test_images/image_capture.jpg')
        #self.path_to_save_result               = rospy.get_param('path_to_save_result',               '../result/gender_recognition_result.jpg')
    ...
    def detection(self):
        ...
        #每个人脸的位置，取中点高度，再所有人脸求平均，比较每个人脸的位置和平均位置的关系
        middle_list = []
        top_list = []
        for i in range(face_num):
            location = result['result']['face_list'][i]['location']
            top = int(location['top'])
            middle = top + int(location['height'])/2
            top_list.append(top)
            middle_list.append(middle)
        average_pos = np.mean(np.array(middle_list))
        for i in range(len(middle_list)):
            if middle_list[i] < average_pos or middle_list[i] == average_pos:
                stand_num = stand_num + 1
            if middle_list[i] > average_pos:
                sit_num = sit_num + 1
        if (sit_num + stand_num) != face_num:
            sit_num = face_num - stand_num
        ...
        gender_message.sit_num    = sit_num
        gender_message.stand_num  = stand_num
        #保存并显示处理后的图片
        print("[INFO] SUCCESSFULLY PUBLISH GENDER DETECTION RESULT")
