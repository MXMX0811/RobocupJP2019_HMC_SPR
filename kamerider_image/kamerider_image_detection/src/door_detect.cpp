...
private:
    ...
    std::string pub_point_num_topic_name;
    std::string path_to_save_image = "/home/zmx/catkin_ws/src/kamerider_navigation/images";
    ...
    */
    ...
    void pclCallback (const sensor_msgs::PointCloud2& msg)
    {
        ...
        std_msgs::Int16 Point_Num;
        Point_Num.data = point_num;
        point_num_pub.publish(Point_Num);
        if (point_num <= 40000)
        {
            door_closed = true;
            close_step ++;
            cout<<"[INFO] close_step = "<<close_step<<endl;
        }
        if (point_num >= 40000)
        {
            door_opened = true;
            open_step ++;
            cout<<"[INFO] open_step = "<<open_step<<endl;
        }
        cout<<"[INFO] Current Point Number is "<<point_num<<endl;
...
public:
    int run (int argc, char** argv)
    {
        ...
        nh.param<std::string> ("path_to_save_image"         , path_to_save_image         , "/home/keaixin/catkin_ws/src/kamerider_image/kamerider_image_detection/result");
        nh.param<std::string> ("pub_point_num_topic_name"   ,pub_point_num_topic_name    ,"/point_num");
        // 定义发布器和订阅器
        door_pub   = nh.advertise<std_msgs::String> (door_detect::pub_door_detect_topic_name, 1);
        point_num_pub = nh.advertise<std_msgs::Int16> (door_detect::pub_point_num_topic_name, 1);
