...
ros::Subscriber person_pose_sub;
ros::Subscriber bar_pose_sub;

std::string NAV_PUBLISH_TOPIC = "/is_reach";
std::string sub_destination_topic_name = "/destination";
std::string sub_person_pose_topic_name = "/person_pose";
std::string sub_bar_pose_topic_name = "/bar_pose";
...
{
    ifstream waypoints;
    waypoints.open("/home/zmx/catkin_ws/src/kamerider_navigation/waypoints.txt", ios::out);
    string data, line;
    vector<string> str; 
    location_pose temp;
...
void personPoseCallback(geometry_msgs::Pose msg)
{
    ifNavigate = true;
    ...
    ROS_INFO("Receive person pose");
}

void barPoseCallback(geometry_msgs::Pose msg)
{
    ifNavigate = true;
    send_flag.data = "in_position_bar";
    goal_pose.position.x = msg.position.x;
    goal_pose.position.y = msg.position.y;
    goal_pose.position.z = msg.position.z;
    goal_pose.orientation.x = msg.orientation.x;
...
int main (int argc, char** argv)
{
    ...
    person_pose_sub = nh.subscribe (sub_person_pose_topic_name, 10, personPoseCallback);
    bar_pose_sub    = nh.subscribe (sub_bar_pose_topic_name,    10, barPoseCallback);
