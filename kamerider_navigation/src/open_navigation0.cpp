...
private:
    ...
    {
        ifstream waypoints;
        waypoints.open("/home/kamerider/catkin_ws/src/kamerider_navigation/waypoints.txt", ios::out);
        string data, line;
        vector<string> str; 
    ...
    void controlCallback(const std_msgs::String& msg)
    {
        ...
        std::cout << "[MSG] received target position " << target_name << std::endl;
...
public:
    int run(int argc, char** argv)
    {
        ...
        speech_sub = nh.subscribe(sub_speech_control_topic_name, 1, &gpsr_navigation::controlCallback, this);
        ...
        while(ros::ok())
        {
            if (start_navigating)
            {
                ...
                if (mc_.getState () == actionlib::SimpleClientGoalState::SUCCEEDED)
                {
                    ...
                    if (target_name == "cupboard")
                    {
                        send_flag.data = "cupboard_arrived";
                    }
                    else if (target_name == "diningroom")
                    {
                        send_flag.data = "diningroom_arrived";
                    }
